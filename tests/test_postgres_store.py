...
async def store(postgres_url):
    ...
    store = PostgreSQLEventStore(
        connection_string=postgres_url, pool_min_size=2, pool_max_size=5
    )
...
async def test_load_events_from_version(store, aggregate_id, sample_events):
    ...
    await store.save_async(aggregate_id, [sample_events[0]])
    await store.save_async(aggregate_id, [sample_events[1]])
    third_event = Event(
        id=str(uuid.uuid4()),
        type="OrderCancelled",
        source="/orders/service",
...
async def test_load_nonexistent_aggregate(store):
    ...
    events = await store.load_async("nonexistent-id")
    assert events == []
...
async def test_concurrent_saves_different_aggregates(store, sample_events):
    ...
    events_2 = await store.load_async(aggregate_2)
...
async def test_ping_failure():
    ...
    store = PostgreSQLEventStore(
        connection_string="postgresql://invalid:5432/nonexistent"
    )
