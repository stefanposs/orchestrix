...
async def test_save_and_load_events(store, aggregate_id, sample_events):
    ...
    # Load events back
    loaded_events = await store.load_async(aggregate_id)
...
async def test_load_nonexistent_aggregate(store):
    ...
    with pytest.raises(ValueError, match="not found"):
        await store.load_async(nonexistent_id)
...
async def test_multiple_aggregates(store):
    ...
    await store.save_async(agg_id_1, events_1)
    await store.save_async(agg_id_2, events_2)

    # Load separately
    loaded_1 = await store.load_async(agg_id_1)
    loaded_2 = await store.load_async(agg_id_2)
...
async def test_event_metadata_preserved(store, aggregate_id):
    ...
    loaded = await store.load_async(aggregate_id)
...
@pytest.mark.asyncio
async def test_sync_methods_not_supported():
    """Test that sync methods are not supported by EventSourcingDBStore."""
    store_instance = EventSourcingDBStore(
        base_url="http://localhost:2113",
    ...
    with pytest.raises(NotImplementedError):
        store_instance.save("agg-id", [])

    with pytest.raises(NotImplementedError):
        store_instance.load("agg-id")
