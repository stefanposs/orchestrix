...
class TestSagaStep:
    ...
    def test_valid_step(self):
        ...
        step = SagaStep(name="test-step", action=action, compensation=compensation)
...
class TestSaga:
    ...
    async def test_compensation_on_failure(self, state_store):
        ...
        saga = Saga(
            "compensation-saga",
            [
                SagaStep(name="step1", action=action1, compensation=compensation1),
    ...
    async def test_is_successful(self, state_store):
        ...
        # Successful saga
        saga1 = Saga("success-saga", [SagaStep(name="step", action=action)], state_store)
        await saga1.initialize()
        await saga1.execute()
    ...
    async def test_multiple_sagas(self, state_store):
        ...
        saga1 = Saga("saga1", [SagaStep(name="step", action=action)], state_store)
        saga2 = Saga("saga2", [SagaStep(name="step", action=action)], state_store)
...
class TestInMemorySagaStateStore:
    ...
    async def test_update_state(self):
        ...
        state2 = SagaState(saga_id="test-saga", saga_type="test", status=SagaStatus.COMPLETED)
