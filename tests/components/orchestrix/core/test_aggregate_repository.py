...
import pytest
from orchestrix.core.eventsourcing.aggregate import AggregateRepository, AggregateRoot
...
class TestAggregateRepositoryAsync:
    ...
    @pytest.mark.asyncio
    async def test_load_async_single_event(self, repository: AggregateRepository[Account]) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_async_multiple_events(
        self, repository: AggregateRepository[Account]
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_async_not_found(self, repository: AggregateRepository[Account]) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_then_modify_then_save(
        self, repository: AggregateRepository[Account]
    ) -> None:
...
class TestAggregateRepositorySync:
    ...
    def test_load_sync_works(self):
        ...
        from orchestrix.infrastructure.memory.store import InMemoryEventStore as SyncStore

        store = SyncStore()
        repository = AggregateRepository[Account](event_store=store)
...
class TestAggregateRootEventReplay:
    ...
    def test_mark_events_committed(self):
        ...
        aggregate = Account()
        aggregate.uncommitted_events = [MoneyDeposited(account_id="test", amount=Decimal("10"))]
