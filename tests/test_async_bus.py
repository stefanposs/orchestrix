...
class TestAsyncMessageBus:
    ...
    @pytest.mark.asyncio
    async def test_async_publish_calls_single_handler(self, async_bus):
        """Test publishing to a single async handler."""
        results = []

        async def handler(msg: Command):
    ...
    @pytest.mark.asyncio
    async def test_async_publish_calls_multiple_handlers(self, async_bus):
        """Test publishing to multiple async handlers."""
        results = []

        async def handler1(msg: Command):
            results.append("handler1")

        async def handler2(msg: Command):
    ...
    @pytest.mark.asyncio
    async def test_handler_exception_does_not_stop_other_handlers(self, async_bus):
        """Test that one handler failing doesn't prevent others from running."""
        results = []

        async def failing_handler(msg: Command):
            msg_text = "Handler failed"
            raise ValueError(msg_text)

        async def success_handler(msg: Command):
            results.append(msg.order_id)
    ...
    @pytest.mark.asyncio
    async def test_all_handlers_fail_raises_error(self, async_bus):
        """Test that if all handlers fail, an error is raised."""

        async def failing_handler_1(msg: Command):
            msg_text = "Handler 1 failed"
            raise ValueError(msg_text)

        async def failing_handler_2(msg: Command):
    ...
    @pytest.mark.asyncio
    async def test_no_handlers_does_not_raise(self, async_bus):
        """Test that publishing with no handlers doesn't raise."""
        # Should not raise
        await async_bus.publish(CreateOrderAsync())

    @pytest.mark.asyncio
    async def test_mixed_success_and_failure_handlers(self, async_bus):
        ...
        async def handler3(msg: Command):
    ...
    @pytest.mark.asyncio
    async def test_only_matching_message_type_handlers_called(self, async_bus):
        ...
        async def order_created_handler(msg: Command):
            results.append("order_created")

        async def order_confirmed_handler(msg: Command):
    ...
    @pytest.mark.asyncio
    async def test_high_message_throughput(self, async_bus):
        """Test async bus with high message volume."""
        counter = {"value": 0}

        async def counting_handler(_: Command):
            counter["value"] += 1
        ...
        tasks = [
            async_bus.publish(CreateOrderAsync())
            for _ in range(100)
        ]
    ...
    @pytest.mark.asyncio
    async def test_subscription_persists_across_publishes(self, async_bus):
