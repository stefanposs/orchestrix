...
import pytest
from orchestrix.core.eventsourcing.projection import InMemoryProjectionStateStore, ProjectionEngine
from orchestrix.core.eventsourcing.versioning import EventUpcaster, UpcasterRegistry
...
class TestIntegration:
    ...
    async def test_complete_order_processing(self) -> None:
        ...
        metrics = PrometheusMetrics(config=MetricConfig(namespace="test", subsystem="order"))
    ...
    async def test_projection_with_versioning(self) -> None:
        ...
        v2_event = await upcaster_registry.upcast(v1_event, "OrderCreated", target_version=2)
    ...
    async def test_metrics_with_async_operations(self) -> None:
        ...
        async def simulate_order_processing() -> None:
            async with metrics.track_async_event_publish(event_type="OrderCreated"):
                await asyncio.sleep(0.001)

            async with metrics.track_async_command_handle(command_type="ConfirmOrder"):
                await asyncio.sleep(0.001)
