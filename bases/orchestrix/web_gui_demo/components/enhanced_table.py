...
def create_simple_table(
    ...
    if not data:
        return dbc.Table([
            ...
            html.Tbody([
                html.Tr([
                    html.Td("No data available", colSpan=len(columns), className="text-center text-muted py-4")
                ])
            ])
        ], bordered=True, hover=True, responsive=True, striped=True, className="table-card", id=table_id)

    table_header = html.Thead(html.Tr([
        html.Th(col, className="fw-bold") for col in columns
    ]))
    ...
    for row in data:
        table_rows.append(
            html.Tr([
                html.Td(row.get(col, "-")) for col in columns
            ])
        )
