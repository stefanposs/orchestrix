...
class EventUpcaster(Generic[SourceEvent, TargetEvent]):
    ...
    @abstractmethod
    async def upcast(self, event: SourceEvent) -> TargetEvent:
    ...
    async def can_upcast(self, event: Event) -> bool:
        ...
        """
        return (
            hasattr(event, "version")
            and getattr(event, "version") == self.source_version
        )
...
class UpcasterRegistry:
    ...
    def register(
        self, event_type: str, upcaster: EventUpcast
    ) -> None:
    ...
    async def upcast(
        self, event: Event, event_type: str, target_version: int
    ) -> Event:
        ...
        if not hasattr(event, "version"):
            raise UpcasterException(
                f"Event {event} missing version attribute for upcasting"
            )

        current_version: int = getattr(event, "version")
        ...
        if current_version > target_version:
            raise UpcasterException(
                f"Cannot downcast from v{current_version} to v{target_version}"
            )
        ...
        while current < target_version:
            ...
            if not upcaster:
                raise UpcasterException(
                    f"No upcaster found for {event_type} "
                    f"v{current}->{next_version}"
                )
    ...
    def get_chain_info(self, event_type: str) -> list[tuple[int, int]]:
        ...
        return [
            (src, tgt)
            for (etype, src, tgt) in self._upcasters.keys()
            if etype == event_type
        ]
