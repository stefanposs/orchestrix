...
class OrderCreatedV1toV2(EventUpcaster[OrderCreatedV1, OrderCreatedV2]):
    ...
    async def upcast(self, event: Event) -> OrderCreatedV2:
        assert isinstance(event, OrderCreatedV1)
...
class OrderCreatedV2toV3(EventUpcaster[OrderCreatedV2, OrderCreatedV3]):
    ...
    async def upcast(self, event: Event) -> OrderCreatedV3:
        assert isinstance(event, OrderCreatedV2)
...
class PaymentProcessedV1toV2(EventUpcaster[PaymentProcessedV1, PaymentProcessedV2]):
    ...
    async def upcast(self, event: Event) -> PaymentProcessedV2:
        assert isinstance(event, PaymentProcessedV1)
...
class TestEventUpcaster:
    ...
    def test_upcaster_invalid_source_version(self) -> None:
        ...
        with pytest.raises(ValueError, match="source_version"):
            ...
            class BadUpcaster(EventUpcaster[Any, Any]):
                def __init__(self) -> None:
                    super().__init__(source_version=0, target_version=2)

                async def upcast(self, event: Event) -> Any:
                    pass
    ...
    async def test_single_upcast(self) -> None:
        ...
        upcaster = OrderCreatedV1toV2()
        event_v1 = OrderCreatedV1(order_id="order-123", customer_id="cust-456", total=99.99)
...
class TestVersionedEvent:
    ...
    def test_versioned_event_creation(self) -> None:
        ...
        event = OrderCreatedV1(order_id="order-123", customer_id="cust-456", total=99.99)
        versioned = VersionedEvent(event=event, version=1, event_type="OrderCreated")
...
class TestUpcasterRegistry:
    ...
    async def test_multiple_event_types(self) -> None:
        ...
        order_v1 = OrderCreatedV1(order_id="order-123", customer_id="cust-456", total=99.99)
        payment_v1 = PaymentProcessedV1(payment_id="pay-789", order_id="order-123", amount=99.99)

        order_v2 = await registry.upcast(order_v1, "OrderCreated", target_version=2)
        payment_v2 = await registry.upcast(payment_v1, "PaymentProcessed", target_version=2)
    ...
    async def test_upcast_failure_handling(self) -> None:
        ...
        class FailingUpcaster(EventUpcaster[Any, Any]):
            def __init__(self) -> None:
                super().__init__(source_version=1, target_version=2)
